...
# ╔═╡ a83518c0-4be8-11eb-29c1-af084a332408
begin
	function interact(photon::Photon, hit::Intersection{Sphere}, 
			num_intersections::Integer, objects::Vector{O}) where {O <: Object}
		theSphere = hit.object
		theSurface = theSphere.s
		
		n̂ = sphere_normal_at(hit.point, theSphere)
		
		reflected_photon = Photon(hit.point, reflect(photon.l, n̂), theSurface.c, photon.ior)
		reflected_photon = step_ray(reflected_photon, objects, num_intersections - 1)

		new_ior = photon.ior == 1.0 ? theSurface.ior : 1.0
		new_dir = refract(photon.l, n̂, photon.ior, new_ior)
		refracted_photon = Photon(hit.point, new_dir, theSurface.c, new_ior)
		# return refracted_photon
		refracted_photon = step_ray(refracted_photon, objects, num_intersections - 1)
		
		# return refracted_photon
		
		final_color = theSurface.r * reflected_photon.c + theSurface.t * refracted_photon.c + theSurface.c * theSurface.c.alpha
		
		Photon(hit.point, photon.l, final_color, photon.ior)
	end
...
main_scene = [
	...
	Sphere([0,50,-100], 20, 
		Surface(0.0, 1.0, RGBA(0,1,0,0.0), 1.0)),
...
let
	...
	ray_trace(main_scene, cam; num_intersections=6)
end
